...
from supplementary_materials.offensive_agent import BeliefBTOffensiveAgent
from supplementary_materials.defensive_agent import BeliefBTDefensiveAgent
#from contest.agents.team_name_1.version3.myTeam import UnifiedBeliefBTAgent
#from myTeam_test import UnifiedBeliefBTAgent
...
class UnifiedBeliefBTAgent(CaptureAgent):
    ...
    def run_away(self, game_state):
        ...
        # NEW: If time critical, just go home immediately
        if self._should_hurry_home(game_state):
            return self._escape_best_option(game_state)
        ...
        # NEW: Critical - if only 2 food left, enemy can win by eating them
        food_left = len(self.get_food_you_are_defending(game_state).as_list())
        carrying = game_state.get_agent_state(self.index).num_carrying
        
        if food_left <= 2 and carrying > 0:
            # We MUST score what we have - enemy can end game
            return self._escape_best_option(game_state)
        
        # NEW: Score pressure - go home if losing and carrying food
        if carrying > 0:
            raw_score = game_state.get_score()
            our_score = raw_score if self.red else -raw_score
            
            enemy_carrying = sum(
                game_state.get_agent_state(opp).num_carrying
                for opp in self.opponents
            )
            
            # If we're losing or tied AND enemy has significant food
            if our_score <= 0 and enemy_carrying >= 5 and carrying >= 3:
                return self._escape_best_option(game_state)
            
            # If score gap is critical
            if our_score <= -5 and carrying >= 2:
                return self._escape_best_option(game_state)
