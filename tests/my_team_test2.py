...
class TeamContext:
    def __init__(self, game_state, team_indices, opponents, is_red, agent):
        ...
        # ---- static layout stats (filled by the leader agent) ----
        self.static_stats_initialized = False
        self.layout_ascii = None
        self.initial_food = None      # list of opponent-food tiles
        self.initial_capsules = None  # list of capsules on opponent side
        self.num_entrances = 0
        self.total_food_initial = 0
        self.food_in_traps = 0
        self.avg_food_danger = 0.0


         # ---- new: geometry / topology stats ----
        self.map_width = 0
        self.map_height = 0
        self.num_walls = 0
        self.num_free_tiles = 0
        self.wall_ratio = 0.0
        self.num_dead_ends = 0
        self.num_corridors = 0
        self.num_junctions = 0

        # trap / pocket stats (enemy side)
        self.num_trap_pockets_off = 0
        self.max_trap_depth_off = 0
        self.avg_trap_depth_off = 0.0
        self.median_trap_depth_off = 0.0
        self.num_trap_tiles_off = 0
        self.trap_tile_ratio_off = 0.0
        self.traps_near_entrances = 0
        self.avg_trap_depth_near_entrances = 0.0

        # entrances
        self.entrance_y_span = 0
        self.avg_entrance_gap = 0.0
        self.max_entrance_gap = 0

        # food vs topology
        self.fraction_food_safe = 0.0
        self.fraction_food_mild_trap = 0.0
        self.fraction_food_deep_trap = 0.0
        self.food_on_dead_ends = 0
        self.food_on_corridors = 0
        self.food_on_junctions = 0
        self.avg_food_cluster_size = 0.0
        self.max_food_cluster_size = 0

        # capsules vs topology
        self.capsules_in_traps = 0
        self.avg_capsule_danger = 0.0
        self.min_capsule_dist_home = 0
        self.min_capsule_dist_enemy_spawn = 0

        # distance-based difficulty
        self.dist_spawn_to_closest_entrance = 0
        self.dist_spawn_to_nearest_food = 0
        self.dist_spawn_to_third_furthest_food = 0
        self.avg_distance_enemy_to_home = 0.0
        self.max_distance_enemy_to_home = 0
...
class UnifiedBeliefBTAgent(CaptureAgent):
    ...
    EXP_LOG_DIR = "exp_two_attackers"
    EXP_LAYOUT_WIN_DIR = os.path.join(EXP_LOG_DIR, "layouts_win")
    EXP_LAYOUT_LOSS_DIR = os.path.join(EXP_LOG_DIR, "layouts_loss")
    EXP_CSV_PATH = os.path.join(EXP_LOG_DIR, "stats.csv")
    _exp_io_initialized = False
    ...
    @classmethod
    def _exp_ensure_io(cls):
        """Create experiment directories and CSV header once per process."""
        if cls._exp_io_initialized:
            return

        os.makedirs(cls.EXP_LAYOUT_WIN_DIR, exist_ok=True)
        os.makedirs(cls.EXP_LAYOUT_LOSS_DIR, exist_ok=True)

        if not os.path.exists(cls.EXP_CSV_PATH):
            with open(cls.EXP_CSV_PATH, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                "timestamp",
                "team_color",
                ...
                # food & traps (existing + refined)
                "initial_food",
                "food_in_traps",
                "avg_food_danger",
                "fraction_food_safe",
                "fraction_food_mild_trap",
                "fraction_food_deep_trap",
                "food_on_dead_ends",
                "food_on_corridors",
                "food_on_junctions",
                "avg_food_cluster_size",
                "max_food_cluster_size",

                # trap structure (enemy side)
                "num_trap_pockets_off",
                "max_trap_depth_off",
                "avg_trap_depth_off",
                "median_trap_depth_off",
                "num_trap_tiles_off",
                "trap_tile_ratio_off",
                "traps_near_entrances",
                "avg_trap_depth_near_entrances",

                # capsules
                "initial_capsules",
                "capsules_in_traps",
                "avg_capsule_danger",
                "min_capsule_dist_home",
                "min_capsule_dist_enemy_spawn",

                # distances
                "dist_spawn_to_closest_entrance",
                "dist_spawn_to_nearest_food",
                "dist_spawn_to_third_furthest_food",
                "avg_distance_enemy_to_home",
                "max_distance_enemy_to_home",
    ...
    def register_initial_state(self, game_state):
        ...
        self.mode = "offense"
         ...
         # ---- EXPERIMENT: static + dynamic stats ----
        team = tuple(sorted(self.get_team(game_state)))
        self._exp_is_leader = (self.index == team[0])

        # leader computes static layout stats once per game
        if self._exp_is_leader and not self.ctx.static_stats_initialized:
            self._init_experiment_static_stats(game_state)
            self.ctx.static_stats_initialized = True

        # leader also resets per-game dynamic counters
        if self._exp_is_leader:
            self._init_experiment_dynamic_stats(game_state)

    def _build_ascii_layout(self, game_state):
        """
        Build a text layout like:
        ...
        Food:  '.'
        Capsules: 'o'
        Spawn positions: '1','2','3','4'
        """
        walls = game_state.get_walls()
        W, H = walls.width, walls.height

        # all food on the board: our food + opponent food
        food = set(self.get_food(game_state).as_list())
        try:
            defended = self.get_food_you_are_defending(game_state).as_list()
            food.update(defended)
        except AttributeError:
            pass

        # all capsules: attacking + defending
        capsules = set(self.get_capsules(game_state))
        try:
            defended_caps = self.get_capsules_you_are_defending(game_state)
            capsules.update(defended_caps)
        except AttributeError:
            pass

        # grid[y][x] with y = 0 at bottom
        grid = []
        for y in range(H):
            row = []
            for x in range(W):
        ...
        # overlay agent spawn positions with digits 1..4
        for idx in range(game_state.get_num_agents()):
            pos = game_state.get_initial_agent_position(idx)
            if pos:
                x, y = pos
                if 0 <= x < W and 0 <= y < H:
                    grid[y][x] = str(idx + 1)

        # dump from top row (H-1) down to 0
        lines = ["".join(grid[y]) for y in reversed(range(H))]
        return "\n".join(lines)

    def _init_experiment_static_stats(self, game_state):
        """Compute per-layout stats (only once per team/game)."""
        self.ctx.layout_ascii = self._build_ascii_layout(game_state)

        walls = game_state.get_walls()
        W, H = walls.width, walls.height
        self.ctx.map_width = W
        self.ctx.map_height = H

        # ---------- 1) Geometry + degree classification ----------
        num_walls = 0
        num_free_tiles = 0
        num_free_enemy = 0
        degrees = {}  # (x, y) -> degree
        ...
        for x in range(W):
            for y in range(H):
                ...
                pos = (x, y)
                num_free_tiles += 1
                if self._is_on_enemy_side_pos(game_state, pos):
                    num_free_enemy += 1
        ...
        self.ctx.num_walls = num_walls
        self.ctx.num_free_tiles = num_free_tiles
        self.ctx.wall_ratio = float(num_walls) / (W * H) if W * H else 0.0
        self.ctx.num_dead_ends = degree_counts[1]
        self.ctx.num_corridors = degree_counts[2]
        self.ctx.num_junctions = degree_counts[3] + degree_counts[4]

        # ---------- 2) Entrances ----------
        entrances = list(self.territory.entrances)
        self.ctx.num_entrances = len(entrances)

        if entrances:
            ys = sorted(y for (_, y) in entrances)
            if len(ys) > 1:
                gaps = [ys[i + 1] - ys[i] for i in range(len(ys) - 1)]
                self.ctx.entrance_y_span = ys[-1] - ys[0]
                self.ctx.avg_entrance_gap = float(sum(gaps)) / len(gaps)
                self.ctx.max_entrance_gap = max(gaps)
            else:
                self.ctx.entrance_y_span = 0
                self.ctx.avg_entrance_gap = 0.0
                self.ctx.max_entrance_gap = 0
        else:
            self.ctx.entrance_y_span = 0
            self.ctx.avg_entrance_gap = 0.0
            self.ctx.max_entrance_gap = 0
        ...
        # ---------- 3) Food & food danger ----------
        initial_food = self.get_food(game_state).as_list()
        initial_capsules = list(self.get_capsules(game_state))

        self.ctx.initial_food = list(initial_food)
        self.ctx.initial_capsules = list(initial_capsules)

        depths = [self.get_food_danger(f) for f in initial_food]
        self.ctx.total_food_initial = len(depths)
        self.ctx.food_in_traps = sum(1 for d in depths if d > 0)
        self.ctx.avg_food_danger = (float(sum(depths)) / len(depths)) if depths else 0.0

        # distribution of food by depth
        if depths:
            safe = sum(1 for d in depths if d == 0)
            mild = sum(1 for d in depths if d in (1, 2))
            deep = sum(1 for d in depths if d >= 3)
            total = float(len(depths))
            self.ctx.fraction_food_safe = safe / total
            self.ctx.fraction_food_mild_trap = mild / total
            self.ctx.fraction_food_deep_trap = deep / total
        else:
            self.ctx.fraction_food_safe = 0.0
            self.ctx.fraction_food_mild_trap = 0.0
            self.ctx.fraction_food_deep_trap = 0.0

        # food vs graph degree type
        food_on_dead_ends = 0
        food_on_corridors = 0
        food_on_junctions = 0
        for f in initial_food:
            deg = degrees.get(f, 0)
            if deg == 1:
                food_on_dead_ends += 1
            elif deg == 2:
                ...
                food_on_junctions += 1
        self.ctx.food_on_dead_ends = food_on_dead_ends
        self.ctx.food_on_corridors = food_on_corridors
        self.ctx.food_on_junctions = food_on_junctions

        # food clustering (radius R = 3 in manhattan distance)
        from contest.util import manhattan_distance  # already imported at top
        cluster_sizes = []
        R = 3
        for f in initial_food:
            cluster = sum(1 for g in initial_food if manhattan_distance(f, g) <= R)
            cluster_sizes.append(cluster)

        if cluster_sizes:
            self.ctx.avg_food_cluster_size = float(sum(cluster_sizes)) / len(cluster_sizes)
            self.ctx.max_food_cluster_size = max(cluster_sizes)
        else:
            self.ctx.avg_food_cluster_size = 0.0
            self.ctx.max_food_cluster_size = 0

        # ---------- 4) Trap structure (enemy side) ----------
        trap_depths_off = []
        num_trap_tiles_off = 0
        for pos in degrees.keys():
            if not self._is_on_enemy_side_pos(game_state, pos):
                continue
            d = self.topology.trap_depth(pos)
            if d > 0:
                num_trap_tiles_off += 1
                trap_depths_off.append(d)

        self.ctx.num_trap_pockets_off = len(getattr(self.topology, "off_pockets", []))
        self.ctx.num_trap_tiles_off = num_trap_tiles_off
        if num_free_enemy > 0:
            self.ctx.trap_tile_ratio_off = float(num_trap_tiles_off) / num_free_enemy
        else:
            self.ctx.trap_tile_ratio_off = 0.0

        if trap_depths_off:
            trap_depths_off_sorted = sorted(trap_depths_off)
            self.ctx.max_trap_depth_off = trap_depths_off_sorted[-1]
            self.ctx.avg_trap_depth_off = (
                float(sum(trap_depths_off_sorted)) / len(trap_depths_off_sorted)
            )
            self.ctx.median_trap_depth_off = trap_depths_off_sorted[
                len(trap_depths_off_sorted) // 2
            ]
        else:
            self.ctx.max_trap_depth_off = 0
            self.ctx.avg_trap_depth_off = 0.0
            self.ctx.median_trap_depth_off = 0.0

        # traps near entrances (within maze distance <= 5)
        near_depths = []
        if entrances:
            for pos in degrees.keys():
                d = self.topology.trap_depth(pos)
                ...
                for ent in entrances:
                    if self.distancer.get_distance(pos, ent) <= 5:
                        near_depths.append(d)
                        break
        self.ctx.traps_near_entrances = len(near_depths)
        self.ctx.avg_trap_depth_near_entrances = (
            float(sum(near_depths)) / len(near_depths) if near_depths else 0.0
        )

        # ---------- 5) Capsules vs topology ----------
        capsules_in_traps = 0
        capsule_depths = []
        for c in initial_capsules:
            d = self.topology.trap_depth(c)
            capsule_depths.append(d)
            if d > 0:
                capsules_in_traps += 1
        self.ctx.capsules_in_traps = capsules_in_traps
        self.ctx.avg_capsule_danger = (
            float(sum(capsule_depths)) / len(capsule_depths) if capsule_depths else 0.0
        )

        home_tiles = self._get_home_tiles(game_state)
        if initial_capsules and home_tiles:
            dists_home = [
                min(self.distancer.get_distance(c, h) for h in home_tiles)
                for c in initial_capsules
            ]
            self.ctx.min_capsule_dist_home = min(dists_home)
        else:
            self.ctx.min_capsule_dist_home = 0

        enemy_spawns = [
            game_state.get_initial_agent_position(opp)
            for opp in self.opponents
            if game_state.get_initial_agent_position(opp) is not None
        ]
        if initial_capsules and enemy_spawns:
            dists_enemy_spawn = [
                min(self.distancer.get_distance(c, s) for s in enemy_spawns)
                for c in initial_capsules
            ]
            self.ctx.min_capsule_dist_enemy_spawn = min(dists_enemy_spawn)
        else:
            self.ctx.min_capsule_dist_enemy_spawn = 0

        # ---------- 6) Distance-based difficulty ----------
        team = tuple(sorted(self.get_team(game_state)))
        attacker_idx = team[1] if len(team) > 1 else team[0]
        attacker_spawn = game_state.get_initial_agent_position(attacker_idx)

        enemy_food = [f for f in initial_food if self._is_on_enemy_side_pos(game_state, f)]

        # spawn -> entrances
        if attacker_spawn and entrances:
            dists_ent = [
                self.distancer.get_distance(attacker_spawn, e) for e in entrances
            ]
            self.ctx.dist_spawn_to_closest_entrance = min(dists_ent)
        else:
            self.ctx.dist_spawn_to_closest_entrance = 0

        # spawn -> food (nearest + third furthest)
        if attacker_spawn and enemy_food:
            dists_food = [
                self.distancer.get_distance(attacker_spawn, f) for f in enemy_food
            ]
            dists_food_sorted = sorted(dists_food)
            self.ctx.dist_spawn_to_nearest_food = dists_food_sorted[0]

            # third furthest, as you wanted (if <3 foods, fall back to max)
            if len(dists_food_sorted) >= 3:
                self.ctx.dist_spawn_to_third_furthest_food = dists_food_sorted[-3]
            else:
                self.ctx.dist_spawn_to_third_furthest_food = dists_food_sorted[-1]
        else:
            self.ctx.dist_spawn_to_nearest_food = 0
            self.ctx.dist_spawn_to_third_furthest_food = 0

        # enemy side -> home distances (avg + max)
        dists_enemy_to_home = []
        if home_tiles:
            for pos in degrees.keys():
                if not self._is_on_enemy_side_pos(game_state, pos):
                    continue
                d = min(self.distancer.get_distance(pos, h) for h in home_tiles)
                dists_enemy_to_home.append(d)

        if dists_enemy_to_home:
            self.ctx.avg_distance_enemy_to_home = (
                float(sum(dists_enemy_to_home)) / len(dists_enemy_to_home)
            )
            self.ctx.max_distance_enemy_to_home = max(dists_enemy_to_home)
        else:
            self.ctx.avg_distance_enemy_to_home = 0.0
            self.ctx.max_distance_enemy_to_home = 0


    def _exp_update_dynamic_stats(self, game_state):
        """Called every turn; leader tracks deaths and food eaten."""
        if not getattr(self, "_exp_is_leader", False):
            return

        team = self.team_indices
        # per-agent respawn detection
        for idx in team:
            st = game_state.get_agent_state(idx)
            pos = st.get_position() if st else None
            spawn = game_state.get_initial_agent_position(idx)

            last_pos = self._exp_last_positions.get(idx)
            last_off = self._exp_last_offensive.get(idx, False)

            if self._exp_seen_first_state:
                # death = teleported to spawn from a non-spawn tile, while previously Pacman
                if (
                    pos is not None
                    ...
                    self._exp_offensive_deaths += 1

            # update memory for next tick
            self._exp_last_positions[idx] = pos
            self._exp_last_offensive[idx] = bool(st and st.is_pacman)

        self._exp_seen_first_state = True

        # track how much opponent food our team has eaten
        cur_food = len(self.get_food(game_state).as_list())
        diff = self._exp_last_food_count - cur_food
        if diff > 0:
            self._exp_team_food_eaten += diff
        self._exp_last_food_count = cur_food


    def _init_experiment_dynamic_stats(self, game_state):
        """Leader-only per-game counters."""
        self._exp_seen_first_state = False
        self._exp_last_positions = {}     # idx -> (x, y)
        self._exp_last_offensive = {}     # idx -> bool (was Pacman)
        self._exp_offensive_deaths = 0
        self._exp_team_food_eaten = 0

        # we track opponent food count to infer how much we ate
        self._exp_last_food_count = len(self.get_food(game_state).as_list())
    ...
    def choose_action(self, game_state):
        ...
        self._exp_update_dynamic_stats(game_state)
    ...
    def _update_mode(self, game_state):
        ...
        modes = self._switch_emergency_on_death(game_state, team, modes)
    ...
    def _switch_emergency_on_death(self, game_state, team, modes):
        # If we already committed to retreat, keep it
        if not self.agents_initialized:
            self.agents_initialized = True
            return modes  # do NOT check for death on the very first turn
        ...
        if self.emergency_retreat:
            for idx in team:
                modes[idx] = "defense"
            return modes

        # Only apply if both are attacking
        all_attacking = all(modes.get(idx) == "offense" for idx in team)
        if not all_attacking:
            return modes
        ...
        for idx in team:
            ...
            start_pos = game_state.get_initial_agent_position(idx)

            if st.num_carrying == 0 and st.get_position() == start_pos:
                died = True
                break

        if died:
            # COMMIT to defense mode
            print("activate emergency")
            self.emergency_retreat = True
            for idx in team:
                modes[idx] = "defense"
    ...
    def _switch_emergency_carry_defense(self, game_state, team, modes):
        ...
        # # Adjust emergency threshold based on topology
        emergency_threshold = self.EMERGENCY_DEF_CARRY_DIFF
        if self.ctx.num_dead_ends > 10:  # Example: high dead-end count
            emergency_threshold -= 1
        
        return_difficulty = 0
        for idx in team:
            if modes.get(idx) != "defense":
                agent_pos = game_state.get_agent_state(idx).get_position()
                if agent_pos:
                    # Maze distance to home
                    home_tiles = self._get_home_tiles(game_state)
                    min_dist_to_home = min(
                        self.distancer.get_distance(agent_pos, home) for home in home_tiles
                    )
                    # Check for ghosts along the path
                    ghosts = self._get_visible_ghosts(game_state)
                    ghost_danger = sum(
                        1 for ghost in ghosts if self.distancer.get_distance(agent_pos, ghost) < 5
                    )
                    return_difficulty += min_dist_to_home + ghost_danger

        # Normalize by team size

        if return_difficulty > 10:  # Example threshold
            emergency_threshold -= 1
    ...
    def _intercept_invader(self, game_state):
        ...
        if self._is_scared(game_state):
            ...
            if threat_info:
                if self._two_defenders_active(game_state):
                    # New behaviour: split roles between two defenders.
                    return self._scared_dual_behavior(game_state, threat_info)
                else:
    ...
    def _should_trap_lockdown(self, game_state):
        ...
        # NEW: if there is a valuable intruder on our side *outside* this trap,
    ...
    def _exp_log_game(self, game_state):
        """Leader writes one CSV row + saves layout to win/lose dir."""
        if not getattr(self, "_exp_is_leader", False):
            return

        self.__class__._exp_ensure_io()

        raw_score = game_state.get_score()
        if raw_score == 0:
            result = "tie"
        else:
            we_won = (raw_score > 0) if self.red else (raw_score < 0)
            result = "win" if we_won else "loss"

        # choose directory for layout
        if result == "win":
            layout_dir = self.EXP_LAYOUT_WIN_DIR
        elif result == "loss":
            layout_dir = self.EXP_LAYOUT_LOSS_DIR
        else:
            layout_dir = self.EXP_LAYOUT_WIN_DIR

        ts = int(time.time())
        rnd = random.randint(0, 999999)
        layout_name = f"{result}_layout_{ts}_{rnd:06d}.txt"
        layout_path = os.path.join(layout_dir, layout_name)

        layout_ascii = getattr(self.ctx, "layout_ascii", None)
        if not layout_ascii:
            layout_ascii = self._build_ascii_layout(game_state)

        with open(layout_path, "w") as f:
            f.write(layout_ascii)

        # ---------- read static stats from ctx ----------
        c = self.ctx

        map_width = getattr(c, "map_width", 0)
        map_height = getattr(c, "map_height", 0)
        num_walls = getattr(c, "num_walls", 0)
        num_free_tiles = getattr(c, "num_free_tiles", 0)
        wall_ratio = getattr(c, "wall_ratio", 0.0)
        num_dead_ends = getattr(c, "num_dead_ends", 0)
        num_corridors = getattr(c, "num_corridors", 0)
        num_junctions = getattr(c, "num_junctions", 0)

        num_entrances = getattr(c, "num_entrances", 0)
        entrance_y_span = getattr(c, "entrance_y_span", 0)
        avg_entrance_gap = getattr(c, "avg_entrance_gap", 0.0)
        max_entrance_gap = getattr(c, "max_entrance_gap", 0)

        initial_food = getattr(c, "total_food_initial", 0)
        food_in_traps = getattr(c, "food_in_traps", 0)
        avg_food_danger = getattr(c, "avg_food_danger", 0.0)
        fraction_food_safe = getattr(c, "fraction_food_safe", 0.0)
        fraction_food_mild_trap = getattr(c, "fraction_food_mild_trap", 0.0)
        fraction_food_deep_trap = getattr(c, "fraction_food_deep_trap", 0.0)
        food_on_dead_ends = getattr(c, "food_on_dead_ends", 0)
        food_on_corridors = getattr(c, "food_on_corridors", 0)
        food_on_junctions = getattr(c, "food_on_junctions", 0)
        avg_food_cluster_size = getattr(c, "avg_food_cluster_size", 0.0)
        max_food_cluster_size = getattr(c, "max_food_cluster_size", 0)

        num_trap_pockets_off = getattr(c, "num_trap_pockets_off", 0)
        max_trap_depth_off = getattr(c, "max_trap_depth_off", 0)
        avg_trap_depth_off = getattr(c, "avg_trap_depth_off", 0.0)
        median_trap_depth_off = getattr(c, "median_trap_depth_off", 0.0)
        num_trap_tiles_off = getattr(c, "num_trap_tiles_off", 0)
        trap_tile_ratio_off = getattr(c, "trap_tile_ratio_off", 0.0)
        traps_near_entrances = getattr(c, "traps_near_entrances", 0)
        avg_trap_depth_near_entrances = getattr(c, "avg_trap_depth_near_entrances", 0.0)

        initial_capsules = len(getattr(c, "initial_capsules", []) or [])
        capsules_in_traps = getattr(c, "capsules_in_traps", 0)
        avg_capsule_danger = getattr(c, "avg_capsule_danger", 0.0)
        min_capsule_dist_home = getattr(c, "min_capsule_dist_home", 0)
        min_capsule_dist_enemy_spawn = getattr(c, "min_capsule_dist_enemy_spawn", 0)

        dist_spawn_to_closest_entrance = getattr(c, "dist_spawn_to_closest_entrance", 0)
        dist_spawn_to_nearest_food = getattr(c, "dist_spawn_to_nearest_food", 0)
        dist_spawn_to_third_furthest_food = getattr(c, "dist_spawn_to_third_furthest_food", 0)
        avg_distance_enemy_to_home = getattr(c, "avg_distance_enemy_to_home", 0.0)
        max_distance_enemy_to_home = getattr(c, "max_distance_enemy_to_home", 0)

        # dynamic stats
        offensive_deaths = getattr(self, "_exp_offensive_deaths", 0)
        team_food_eaten = getattr(self, "_exp_team_food_eaten", 0)

        with open(self.EXP_CSV_PATH, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                ts,
                "red" if self.red else "blue",
                result,
                raw_score,
