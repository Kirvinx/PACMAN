...
class UnifiedBeliefBTAgent(CaptureAgent):
    ...
    def _assign_default_roles(self, game_state, team):
        ...
        Exception: if our territory has only a single entrance, or exactly two
        entrances that are directly adjacent, we treat it as a single choke
        point and keep one dedicated defender (lowest index).
        """
        modes = {}
        ...
        if is_single_choke():
            # 1 attacker, 1 defender (lowest index def)
            defender = team[0]
            for idx in team:
                modes[idx] = "offense"
    ...
    def _switch_emergency_carry_defense(self, game_state, team, modes):
        ...
        Uses only num_carrying (food “in mouth”), not the scoreboard.
        """
        # Our carried food
        our_carry = 0
        for idx in team:
        ...
        if intruder_here and diff >= self.EMERGENCY_DEF_CARRY_DIFF:
    ...
    def _intercept_invader(self, game_state):
        ...
        - If we are scared, use the old "avoid scared invader" behaviour.
        ...
        """
        # When scared, we don't chase; we avoid.
    ...
    def final(self, game_state):
        ...
        Use this to log experiment stats and clear only this game's
        shared context so it doesn't leak into the next game.
        """
        try:
        ...
        print(f"[LOGGING] final() called on agent {self.index}")

        if self.index == leader:
            # 1) Log game stats (safe to do before clearing)
            self._exp_log_game(game_state)

            # 2) Remove only this game's context, not all contexts
            game_id = id(game_state.data)
