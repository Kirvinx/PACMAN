...
class MapTopologyAnalyzer:
    ...
    def __init__(self, walls):
        ...
        #self._attach_doors_to_off_pockets()
    ...
    def _attach_doors_to_off_pockets(self):
        """
        Assign articulation tiles (doors) to the off-pocket they lead into.

        Invariants after this:
        - If a door leads into an off-pocket i, then:
            pocket_id[door] == i
            door is included in pockets[i]
        - Doors can still be adjacent to the main pocket, but we consider
            them "belonging" to the trap side.
        """
        if not self.off_pockets:
            return

        for i in self.off_pockets:
            for door in self.pocket_exits[i]:
                # Skip if door already has a pocket assignment
                if door in self.pocket_id:
                    continue

                # Make sure this door is actually adjacent to this pocket
                # (defensive, though it should always hold)
                if any(self.pocket_id.get(nb) == i for nb in self.neighbors.get(door, [])):
                    self.pocket_id[door] = i
                    self.pockets[i].append(door)
