...
class Colors:
    ...
    # Two-exit region overlay colors (use light cyan/green/purple)
    TWO_EXIT_EXITS = '\033[1m\033[38;5;51m'     # bright cyan for exits
    TWO_EXIT_INTERIOR = '\033[38;5;48m'         # green for interior tiles
...
def print_legend():
    ...
    print(f"  {Colors.TWO_EXIT_EXITS}E{Colors.RESET}   = Two-exit door tile")
    print(f"  {Colors.TWO_EXIT_INTERIOR}o{Colors.RESET}   = Two-exit interior region")
    print("="*60 + "\n")

def main():
    with open("defaultCapture.lay") as f:
    ...
    two_exit_regions = getattr(analyzer, "two_exit_regions", [])
    two_exit_tiles = set()
    exit_tiles = set()
    for reg in two_exit_regions:
        exit_tiles.update(reg["exits"])
        two_exit_tiles.update(reg["tiles"])
    ...
    for y in range(walls.height - 1, -1, -1):
        ...
        for x in range(walls.width):
            ...
            if walls[x][y]:
                ...
                if pos in exit_tiles:
                    row += f"{Colors.TWO_EXIT_EXITS}E{Colors.RESET}"
                    continue
                elif pos in two_exit_tiles:
                    row += f"{Colors.TWO_EXIT_INTERIOR}o{Colors.RESET}"
                    continue
